...
function FdmAnimation() {
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      setExtruderX((prev) => {
        const next = prev + direction * 0.035;
        if (next >= 1) {
          setDirection(-1);
  ...
  // Statue of Liberty shape path (simplified silhouette)
  const statuePath = `
    M ${printAreaX + printAreaW / 2} ${bedY - 160}
    L ${printAreaX + printAreaW / 2 - 8} ${bedY - 150}
    L ${printAreaX + printAreaW / 2 - 6} ${bedY - 140}
    L ${printAreaX + printAreaW / 2 - 10} ${bedY - 120}
    L ${printAreaX + printAreaW / 2 - 15} ${bedY - 80}
    L ${printAreaX + printAreaW / 2 - 12} ${bedY - 40}
    L ${printAreaX + printAreaW / 2 - 18} ${bedY - 20}
    L ${printAreaX + printAreaW / 2 - 25} ${bedY}
    L ${printAreaX + printAreaW / 2 + 25} ${bedY}
    L ${printAreaX + printAreaW / 2 + 18} ${bedY - 20}
    L ${printAreaX + printAreaW / 2 + 12} ${bedY - 40}
    L ${printAreaX + printAreaW / 2 + 15} ${bedY - 80}
  ...
  return (
    <div className="relative mx-auto max-w-lg">
      ...
      >
        ...
        <svg width="100%" viewBox={`0 0 ${svgW} 280`} style={{ display: "block" }}>
          <defs>
            {/* Mask for the statue shape */}
            <mask id="statue-mask">
              <rect x="0" y="0" width={svgW} height="300" fill="black" />
              <path d={statuePath} fill="white" />
            </mask>
          ...
          {/* Background grid */}
          <rect x="0" y="0" width={svgW} height="280" fill="url(#grid-pattern)" />

          {/* Vertical frame rails */}
          <rect x="30" y="20" width="6" height={bedY - 20} rx="3" fill="#2b2b2b" stroke="#000" strokeWidth="1" />
          <rect x={svgW - 36} y="20" width="6" height={bedY - 20} rx="3" fill="#2b2b2b" stroke="#000" strokeWidth="1" />
          ...
          >
            ...
            {/* Nozzle */}
            <polygon
              points={`${extruderW / 2 - 6},${extruderYPx + extruderH - 6} ${extruderW / 2 + 6},${extruderYPx + extruderH - 6} ${extruderW / 2},${extruderYPx + extruderH + 8}`}
              fill="var(--accent)"
              stroke="#000"
              strokeWidth="1.5"
            />
            {/* Hot tip glow with animation */}
            <motion.circle
              cx={extruderW / 2}
              cy={extruderYPx + extruderH + 6}
              r="4"
              fill="#fff"
              animate={{
                opacity: [0.6, 1, 0.6],
          ...
          {/* Masked printed object - only visible within statue shape */}
          <g mask="url(#statue-mask)">
            {/* Printed layers */}
            {Array.from({ length: currentLayer }).map((_, i) => {
              const y = bedY - (i + 1) * layerH;
              const opacity = 0.2 + (i / totalLayers) * 0.6;
              return (
                <motion.rect
                  key={i}
                  x={printAreaX}
        ...
        {/* Floating badges with neobrutalist style */}
        <motion.div
          animate={{ y: [0, -6, 0] }}
          transition={{ repeat: Infinity, duration: 2.8, ease: "easeInOut" }}
          className="absolute -right-4 -top-4 rounded-xl border-2 border-black bg-white px-4 py-2 text-xs font-bold shadow-[4px_4px_0_#000]"
          style={{
        ...
        <motion.div
          ...
          className="absolute -bottom-4 -left-4 rounded-xl border-2 border-black px-4 py-2 text-xs font-bold shadow-[4px_4px_0_#000]"
          style={{
            background: "#3ecf8e",
            color: "#000",
          }}
    ...
    <section className="relative overflow-hidden px-6 py-28 text-center">
      ...
      <div className="relative mx-auto max-w-4xl">
        ...
        >
          <Link
            href="/login"
            className="neo-brutal-btn inline-flex items-center gap-2 rounded-lg px-8 py-4 text-base font-bold text-white"
            style={{
              background: "var(--accent)",
          ...
          <a
            href="#cennik"
            className="neo-brutal-btn inline-flex items-center gap-2 rounded-lg border-2 border-black bg-white px-7 py-4 text-base font-bold text-black"
...
function FeaturesSection() {
  const iconColors = ["#9d6bff", "#3ecf8e", "#5b8def", "#e0944a"];
      ...
      <div className="mx-auto max-w-6xl">
        ...
        <StaggerGrid className="mt-12 grid gap-5 sm:grid-cols-2 lg:grid-cols-4">
          {features.map((f, idx) => (
            <div
              key={f.title}
              className="neo-brutal-card group rounded-xl border-2 border-black p-6 transition-all duration-300"
              style={{
                background: "var(--bg-card)",
                boxShadow: "6px 6px 0 #000"
              }}
            >
              <div
                className="mb-4 inline-flex h-12 w-12 items-center justify-center rounded-lg border-2 border-black transition-all group-hover:scale-105"
                style={{ background: iconColors[idx % iconColors.length] }}
              >
                <f.icon className="h-6 w-6" style={{ color: "#fff" }} />
              </div>
