...
import { Plus, Trash2, Eye, ToggleLeft, ToggleRight, X, ClipboardList, Pencil } from "lucide-react";
...
export function FormsManager() {
  ...
  async function handleCreate() {
    ...
    try {
      const body = JSON.stringify({ title, description, visibility, allowSkip, active, questions });
      const r = editingId
        ? await fetch(`/api/forms/${editingId}`, { method: "PATCH", headers: { "Content-Type": "application/json" }, body })
        : await fetch("/api/forms", { method: "POST", headers: { "Content-Type": "application/json" }, body });
      if (!r.ok) { const d = await r.json(); setError(d.error ?? "Błąd."); return; }
      setCreating(false); resetForm(); await load();
    } finally { setSaving(false); }
  }

  function resetForm() { setTitle(""); setDescription(""); setVisibility("ALL"); setAllowSkip(true); setActive(true); setQuestions([]); setError(null); setEditingId(null); }
  ...
  async function openEdit(f: Form) {
    setError(null);
    const r = await fetch(`/api/forms/${f.id}`);
    if (!r.ok) return;
    ...
    setAllowSkip(form.allowSkip);
    setActive(form.active);
    setQuestions((form.questions ?? []).map((q: Question) => ({ id: q.id, text: q.text, type: q.type, options: q.options ?? undefined, required: q.required, order: q.order })));
    setCreating(true);
  }
  ...
  if (creating) return (
    <div className="mx-auto max-w-5xl space-y-4 p-6 md:p-8">
      <div className="flex items-center gap-3">
        ...
        <h2 className="font-display text-lg font-semibold text-[var(--text-primary)]">{editingId ? "Edytuj formularz" : "Nowy formularz"}</h2>
      </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
          <div className="space-y-1"><label className="text-xs font-semibold text-[var(--text-muted)] uppercase tracking-wide">Widoczność</label>
            ...
            <p className="text-xs text-[var(--text-muted)] mt-1">
              {visibility === "ALL" && "Formularz zobaczy każdy zalogowany użytkownik, niezależnie od daty dołączenia."}
              {visibility === "ACTIVE" && "Formularz zobaczą tylko użytkownicy, którzy dołączyli przed datą utworzenia tego formularza."}
              {visibility === "NEW" && "Formularz zobaczą tylko użytkownicy, którzy dołączą po dacie utworzenia tego formularza."}
            </p>
          </div>
      ...
      <div className="flex gap-3">
        <button onClick={handleCreate} disabled={saving} className="rounded-lg bg-[var(--accent)] px-5 py-2 text-sm font-semibold text-white hover:opacity-90 disabled:opacity-50 transition-opacity">{saving ? "Zapisywanie..." : editingId ? "Zapisz zmiany" : "Utwórz formularz"}</button>
  ...
  return (
    <div className="mx-auto max-w-5xl space-y-4 p-6 md:p-8">
      <div className="flex items-center justify-between">
        ...
        <button onClick={() => { resetForm(); setCreating(true); }} className="flex items-center gap-2 rounded-lg bg-[var(--accent)] px-4 py-2 text-sm font-semibold text-white hover:opacity-90 transition-opacity"><Plus className="h-4 w-4" /> Nowy formularz</button>
      </div>
              ...
              {forms.map(f => (
                <tr key={f.id} className="border-b border-[var(--border-subtle)] last:border-0 hover:bg-[var(--bg-elevated)] transition-colors">
                  ...
                  <td className="px-4 py-3 text-right"><div className="flex items-center justify-end gap-1">
                    ...
                    <button onClick={() => openEdit(f)} className="rounded-md p-1.5 text-[var(--text-muted)] hover:text-[var(--accent)] hover:bg-[var(--bg-elevated)]" title="Edytuj"><Pencil className="h-4 w-4" /></button>
